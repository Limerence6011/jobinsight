...
if __name__ == "__main__" and __package__ is None:
    # 获取项目根目录（jobinsight_project）
    current_file = Path(__file__).resolve()
    
    # 方法1：从当前文件向上查找 jobinsight_project 目录
    for parent in current_file.parents:
        if parent.name == "jobinsight_project" and (parent / "jobinsight").exists():
        ...
        # 方法2：从当前工作目录查找
        cwd = Path.cwd()
        if (cwd / "jobinsight").exists():
            sys.path.insert(0, str(cwd))
            os.chdir(str(cwd))
        elif (cwd.parent / "jobinsight").exists():
            # 如果在 jobinsight 子目录中
            sys.path.insert(0, str(cwd.parent))
            os.chdir(str(cwd.parent))
        else:
            # 方法3：从当前文件向上查找包含 jobinsight 目录的父目录
            for parent in current_file.parents:
                if (parent / "jobinsight").exists():
                    sys.path.insert(0, str(parent))
                    os.chdir(str(parent))
                    break
